...
namespace PavilionScratchpad
{
		...
		, IComparable<BigDecimal>
	{
		...
		public BigDecimal Floor() => Truncate(BigDecimal.NumberOfDigits(Mantissa) + Exponent);

		public static int NumberOfDigits(BigInteger value) =>
			// do not count the sign
			//return (value * value.Sign).ToString().Length;
			// faster version
			(int)Math.Ceiling(BigInteger.Log10(value * value.Sign));
		...
		public static BigDecimal operator *(BigDecimal left, BigDecimal right) => new(left.Mantissa * right.Mantissa, left.Exponent + right.Exponent);
		...
		public override bool Equals(object? obj)
		{
			...
			return obj is BigDecimal bigDecimal && Equals(bigDecimal);
		}
